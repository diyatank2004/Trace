...
class Sprint(Base):
    ...
    status = Column(Enum(SprintStatus, native_enum=False, omit_aliases=False), nullable=False, default=SprintStatus.FUTURE)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
...
class Task(Base):
    ...
    # Bidirectional backref linkage configuration
    ...
    subtasks = relationship("Task", cascade="all, delete-orphan", remote_side=[id], single_parent=True)
